      ...
      if (command === 'CommonEXP') {
		...
		if(Orgasm > 0){
			...
			if($gameParty.inBattle()){
				$gameActors.actor(1).gainHp(-Math.round($gameActors.actor(1).mhp * Math.max(Orgasm * 16 - $gameActors.actor(1)._tp,0) / 250));
			}else{
				$gameActors.actor(1).gainHp(-Math.round(Math.min($gameActors.actor(1).mhp * Math.max(Orgasm * 16 - $gameActors.actor(1)._tp,0) / 250,$gameActors.actor(1).hp - 1)));
			}
			...
			$gameActors.actor(1).gainSilentTp(-Math.round(16 * Orgasm));
			//事件附加余韵
		...
		if(CallSemen){
			...
			if($gameActors.actor(1)._classId > 2){
				$gameActors.actor(1).gainMp((PlayNum + randomNum) * $gameActors.actor(1).mmp * 0.1);
				$gameActors.actor(1).gainHp((PlayNum + randomNum) * $gameActors.actor(1).mhp * 0.15);
				$gameActors.actor(1).gainSilentTp((PlayNum + randomNum) * 10);
			}
			if(PlayID == "膣内射精"){
