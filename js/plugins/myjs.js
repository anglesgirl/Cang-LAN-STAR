(function () {
    ...
    Game_Interpreter.prototype.pluginCommand = function (command, args) {
        ...
        if (command === 'CMskill') {
            var skillcommand = args[0];
            var learnedSkills = [];
            for (var i = 1; i <= 129; i++) {
                if (actor.hasSkill(i)) {
                    learnedSkills.push(i);
                    $gameVariables._data[3721] = learnedSkills;
                }
            }
            for (var i = 309; i <= 667; i++) {
                if (actor.hasSkill(i)) {
                    learnedSkills.push(i);
                    $gameVariables._data[3721] = learnedSkills;
                }
            ...
            switch (skillcommand) {
                case 0:
                    //遗忘
                    learnedSkills = $gameVariables._data[3721];
    ...
    function show(folder, ID, number, origin, X, Y, scale) {
        const FileName = folder + '/' + ID;
        $gameScreen.showPicture(number, FileName, origin, X, Y, scale, scale, 255, 0);
    }
